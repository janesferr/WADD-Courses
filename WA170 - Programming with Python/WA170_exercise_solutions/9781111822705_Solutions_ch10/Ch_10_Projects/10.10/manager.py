...
Based on solutions to Projects 8.5 and 10.9
...
The manager can also start and shut down an ATM server for the bank.
"""
from bank import Bank, SavingsAccount
from atmserver import ATMServer

class Manager(object):
    """This class represents terminal-based bank manager functions."""

    def __init__(self, bank):
        self._atmServer = None
        self._account = None
        self._bank = bank
        ...
        self._methods["7"] = self._startATM
        self._methods["8"] = self._stopATM
        self._methods["9"] = self._quit
    ...
    def run(self):
        ...
        while True:
            ...
            print("7  Start the ATM")
            print("8  Stop the ATM")
            print("9  Quit\n")
    ...
    def _startATM(self):
        """Starts the ATM server."""
        self._atmServer = ATMServer(self._bank)
        self._atmServer.start()
        print("ATM server started.")

    def _stopATM(self):
        """Shuts down the ATM server."""
        if self._atmServer != None:
            self._atmServer.quit()
